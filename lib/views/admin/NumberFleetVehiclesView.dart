...
class Numberfleetvehiclesview extends StatelessWidget {
  final int userId;
  final TextEditingController _controller = TextEditingController();

  Numberfleetvehiclesview({required this.userId});
  ...
  String generateFleetRegistration(int nextNumber) {
    ...
    return 'V$firstLetter$secondLetter${nextNumber.toString().padLeft(2, '0')}';
  }
  ...
  Future<void> submitFleet(BuildContext context) async {
    if (_controller.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Ingrese una cantidad de vehiculos.')),
      );
      return;
    }
    ...
    final db = await DBConnection().getConnection();
    await db.query(
      'INSERT INTO flotillas (matricula_flotilla, tipo_flotilla, cantidad_flotilla, id_usuario_flotilla, fecha_creacion_flotilla) VALUES (?, 1, ?, ?, ?)',
      [fleetRegistration, '$fleetCount Vehiculo(s)', userId, creationDate],
    );
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              '¿Cuántos vehiculos pertenecen a su flotilla?',
